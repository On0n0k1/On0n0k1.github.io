...
function setCanvasVisible(canvasVisible: string) {
...
function setCanvasInvisible(canvasInvisible: string) {
...
function getState(canvasVisible: string): boolean {
...
interface FPSCheckboxProps {
  id?: string
}
// Renders a checkbox that controls when fps is enabled or not.
export default function FPSCheckbox(props: FPSCheckboxProps) {
  ...
  const [isChecked, setIsChecked] = useState(getState(canvasVisible));

  useEffect(() => {
    // The instruction after && is only executed when the condition behind is true.
    isChecked && setCanvasVisible(canvasVisible);
    !isChecked && setCanvasInvisible(canvasInvisible);
  }, [isChecked]);
